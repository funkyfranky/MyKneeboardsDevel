...
% some info
\pdfinfo{%
  /Title    (AV-8B Harrier Procedures)
  /Author   (funkyfranky)
  /Creator  ()
  /Producer ()
  /Subject  ()
...
% Takeoff Checklist
\begin{itemize}
\item[] {\LARGE\textbf{Takeoff Checklist}}
\item[\yi] Nozzle rotation airspeed\dotfill\button{AS REQUIRED}
\begin{itemize}
 \item[\yi] Master mode\dotfill\button{V/STOL}
 \item[\yi] ODU select\dotfill\button{NRAS}
 \item[\yi] UFC enter\dotfill\button{NRAS VALUE}
...
\begin{itemize}
 ...
 \item[\yi] UFC enter\dotfill\button{14}
\end{itemize}
\item[\gi] STO stop\dotfill\button{AS REQUIRED}
\item[\yi] Trim\dotfill\button{2\Deg\;ND}
\item[\yi] Flaps\dotfill\button{AS REQUIRED}
\item[\yi] Warning/caution lights\dotfill\button{OUT}
\item[\yi] Engine\dotfill\button{CHECK}
\begin{itemize}
 \item[\yi] MPCD select\dotfill\button{ENG/BOX ACCEL}
 \item[\yi] Engine RPM\dotfill\button{IDLE $\rightarrow$ 60\%}
...
\begin{itemize}
 ...
 \item[\gi] Engine RPM\dotfill\button{RESET TO TOP END OF ACCELERATION BAND}
\end{itemize}
\item[\yi] Nozzle/flaps/duct pressure\dotfill\button{CHECK}
\begin{itemize}
 \item[\gi] Nozzles momentrarily\dotfill\button{STO stop (or 50\%)}
 \item[\yi] Flaps\dotfill\button{CHECK PROPER ANGLE BASED ON FLAPS MODE}
 \item[\yi] Duct pressure\dotfill\ok{$\sim$45\;PSI}
 \item[\gi] Nozzles\dotfill\button{TO position}
\end{itemize}
\end{itemize}